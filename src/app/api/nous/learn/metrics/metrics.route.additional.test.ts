  ...
  describe("GET Endpoint", () => {
    it("should validate metric types", async () => {
      const request = new NextRequest(
        "http://localhost:3000/api/nous/learn/metrics?metrics=INVALID_METRIC"
      );
    ...
    it("should handle multiple metrics", async () => {
      ...
      vi.mocked(prisma.learningMetric.findMany).mockResolvedValue(mockMetrics);
      vi.mocked(prisma.learningMetric.count).mockResolvedValue(2);

      const request = new NextRequest(
        "http://localhost:3000/api/nous/learn/metrics?metrics=FEEDBACK_SCORE,ENGAGEMENT_RATE"
      );
      const response = await GET(request);
      const data = await response.json();
