...
describe("Learning Events Route Handler", () => {
  ...
  describe("GET /api/nous/learn/events", () => {
    it("should fetch events with default parameters", async () => {
      ...
      vi.mocked(mockPrisma.learningEvent.findMany).mockResolvedValueOnce([1]); // DB connection test
      vi.mocked(mockPrisma.learningEvent.findMany).mockResolvedValueOnce(mockEvents);
