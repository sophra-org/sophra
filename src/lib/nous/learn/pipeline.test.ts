...
describe("LearningPipeline", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(mockPrisma.modelState.findUnique).mockResolvedValue(
      mockModelState
    );
  });
  ...
  describe("trainAndDeploy", () => {
    ...
    it("should handle training errors", async () => {
      vi.mocked(mockRegistry.registerModel).mockRejectedValueOnce(
        new Error("Registration failed")
      );

      await expect(pipeline.trainAndDeploy(mockEvents)).rejects.toThrow(
        "Registration failed"
      );
    });
  });
  ...
  describe("OpenAI training", () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(mockRegistry.getModel).mockResolvedValue({
        id: mockModelId,
        configId: JSON.stringify({
          type: "OPENAI_FINE_TUNED",
          hyperparameters: {},
    ...
    it("should train OpenAI model successfully", async () => {
      ...
      const result = await pipeline.trainAndDeploy([event], {
        type: ModelType.OPENAI_FINE_TUNED,
        hyperparameters: {},
        features: [],
        trainingParams: {},
